...
# 示例运行（请根据实际任务嵌入替换以下模拟数据）
if __name__ == "__main__":
    num_layers = 4
    num_tasks = 10
    input_dim = 384
    # 任务嵌入：每层 20 个任务 × 384 维嵌入
    train_embed = {}
    task_name = 'cw10'
    ...
    for number in range(4):
        ...
        for task_idx, dict_task in enumerate(seq_tasks):
            task_e = task_encoder.encode(dict_task["hint"])[np.newaxis, :]
            task_embeddings.append(task_e[0])  # (20,384)
        train_embed[number] = np.array(task_embeddings) #(4,20,384)
    # print(train_embed[0].shape) 
