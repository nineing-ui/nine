...
flags.DEFINE_string('wandb_project_name', "test_overlap_params_situation", "The wandb's project name.")
...
def main(_):
    ...
    for task_idx, dict_task in enumerate(seq_tasks):
        ...
        # checkpoint_path_name = 'stored_agent_new_mechanism/440/'
        # # checkpoint_path_name = 'stored_agent_and_cumul_masks_and_grad_masks/330/0
        # model_task_id = 1
        # check_point_path = checkpoint_path_name + str(model_task_id) + '/actor.pkl'
        # agent.actor = agent.actor.load(check_point_path)
        # dict_task = seq_tasks[model_task_id]
        # eval_envs = []
        # eval_envs.append(get_single_env(dict_task['task'], FLAGS.seed, randomization=FLAGS.env_type))
        # with open(checkpoint_path_name + str(model_task_id) + '/cumul_masks.pkl', 'rb') as f:
        #     agent.cumul_masks = pickle.load(f)
        # with open(checkpoint_path_name + str(model_task_id) + '/param_masks.pkl', 'rb') as f:
        ...
        #     {'params': agent.actor.params}, b['masks']
        # )
        # overlap_params = tree_map(lambda x, y: x * y, current_grad_masks, unfreeze(agent.param_masks))
        # layer_name_list = ['backbones_0', 'backbones_1', 'backbones_2', 'backbones_3', 'mean_layer']
        # overlap_params_number = {}
        # forward_params_number = {}
        # total_overlap_params = 0
        ...
        # for layer_name in layer_name_list:
        #     overlap_params_number[layer_name] = overlap_params[(layer_name, 'kernel')].flatten().astype(bool).astype(int).sum()
        #     forward_params_number[layer_name] = current_grad_masks[(layer_name, 'kernel')].flatten().astype(bool).astype(int).sum()
        #     total_overlap_params += overlap_params_number[layer_name]
        #     total_forward_params += forward_params_number[layer_name]
        # print("total_overlap_params: ", total_overlap_params)
        # print("total_forward_params: ", total_forward_params)
        # print("total_overlap_params_ratio: ", total_overlap_params / total_forward_params)
